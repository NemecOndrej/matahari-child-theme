...
function deployFiles(pathFile, pathMap) {
  ...
  if (config.protocol == 'ftp') {
      ...
      const conn = ftp.create( {
          ...
          parallel: 10,
          reload:   true,
          remotePath: config.remotePath+"wp-content/themes/"+themeFolderName+"/",
          debug:    function(d){console.log(d);},
          log:      gutil.log
      });
      return gulp.src( globs, { base: '.', buffer: false } )
          .pipe( conn.newer( config.remotePath+"wp-content/themes/"+themeFolderName+"/" ) ) // only upload newer files
          .pipe( conn.dest( config.remotePath+"wp-content/themes/"+themeFolderName+"/" ) );
  } else {
      // SFTP version
      const conn = sftp({
              host: config.host,
